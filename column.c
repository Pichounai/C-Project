...
COLUMN *create_column(char *title) {
    COLUMN column;
    COLUMN *pointer_column = &column;
    pointer_column->title = (char*) malloc(strlen(title) * sizeof(char));
    pointer_column->title = title;
    pointer_column->data = NULL;
    pointer_column->physical_size = 0;
    pointer_column->logical_size = 0;
...
int insert_value(COLUMN *col, int value) {
    if (col->logical_size == col->physical_size) {
        ...
        if (col->data == NULL) {
            ...
            test_alloc = realloc(col->data, col->physical_size + REALOC_SIZE * sizeof(int));
        }
        if (test_alloc == NULL) {
            return 0;
        }
    ...
    col->logical_size = col->logical_size + 1;
    return 1;
}
...
void print_col(COLUMN *col) {
    int value_nb;
    for (value_nb = 0; value_nb < col->logical_size; value_nb = value_nb + 1) {
